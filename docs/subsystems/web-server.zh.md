...
处理过程中发生拒绝的请求会记录为警告并隔离处理，绝不导致进程退出。被归类为客户输入错误的拒绝（`BadRequestError`，由 route 或 fallback handler 在遇到格式错误的百分号转义或无法解析的请求目标时抛出）会应答 400；任何其他拒绝（内部 route 或 fallback 失败、静态读取失败、客户端在请求体中途断开）都应答 500；响应头已发出时则销毁 socket。dispose（资源释放）把 `close()` 与 `closeAllConnections()` 配对使用，因为处理器可能像 SSE（Server-Sent Events）那样保持响应打开，而这类连接永远不会自行结束；没有强制关闭，拆卸就会挂起。该包从不打印输出：URL 行归 shell 所有。逐包运维细节（含开发模式的 bundle 监视流水线）留在 [README](../../packages/host/webserver/README.md) 中。
