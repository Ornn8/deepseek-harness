...
describe('CI workflow', () => {
  ...
  it('keeps a required Wine Windows job, a non-blocking native Windows job with failover, and a master-only standby', () => {
    ...
    expect(windowsNative['runs-on']).toContain('github.repository_owner')
    ...
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      ...
      expect(job['runs-on']).toContain('github.repository_owner')
...
describe('Python release workflows', () => {
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(manylinuxAddon)).toContain('pnpm rebuild -r node-pty')
...
describe('Agent automation workflows', () => {
  const controllerSha = '76f1148f4e4228099068316f369bc71475c6c968'
  ...
  it('separates review publication, repair wakeups, landing, and model-free health', () => {
    ...
    expect(workflowEvent(issues, 'issues')).toEqual({
