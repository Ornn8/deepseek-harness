...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` (`@deepseek-ai/dsh-desktop`) is the standalone GUI shell: an Electron main process that boots the same `web` profile composition and loads the loopback HTTP origin the webserver serves, exactly like a browser ([standalone GUI architecture note](2026-08-14-desktop-gui-minimum-change-standalone-architecture.md)); an IPC fetch carrier remains a deferred transport decision.
```
...
1. **Pick a fetch impersonation**: browser same-origin HTTP / in-process `host.handler.fetch` injection / your own transport-aspect subclass (e.g. a future IPC carrier, see the "Subclass table" below).
...
| IPC bridge subclass (hypothetical — deferred; the shipped Electron shell uses the loopback HTTP carrier, so no such subclass exists) | an Electron shell | IPC serialization round trip | would swap only doFetch; contract and base class unchanged |
