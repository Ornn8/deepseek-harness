...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）是独立 GUI 外壳：一个启动相同 `web` profile 组合并加载 webserver 所服务的回环 HTTP 源的 Electron 主进程，与浏览器完全一致（[独立 GUI 最小改动架构说明](2026-08-14-desktop-gui-minimum-change-standalone-architecture.md)）；IPC fetch 载体仍是待定的传输决策。
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP / 进程内 `host.handler.fetch` 注入 / 自写传输切面子类（如将来的 IPC 载体，见下文「子类表」）。
...
| IPC 桥子类（假想——已推迟；已交付的 Electron 外壳使用回环 HTTP 载体，因此尚无此形态） | Electron 壳 | IPC 序列化往返 | 只需换 doFetch，约定/基类零改 |
