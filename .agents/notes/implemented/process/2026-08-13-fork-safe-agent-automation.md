...
After a push to the repository default branch, reconciliation compares each same-repository PR's recorded base commit with the current default-branch commit, updates a stale base regardless of GitHub's transient mergeability state, and dispatches review only for current pairs without trusted exact-pair evidence. GitHub suppresses ordinary workflow recursion after its job token changes a label, so backlog, reconciliation, and bounded recovery use explicit `repository_dispatch` events; backlog may reclaim an `agent/dsh` Issue that has no closing PR or terminal failure, while workflow concurrency and the stable work-request identity prevent a second model turn. The protected default-branch listeners and pinned reusable controller revalidate the live Issue or exact PR pair before invoking a model. The manual health workflow checks each configured worker on its own runner plus the pinned controller and GitHub access without invoking a model.
