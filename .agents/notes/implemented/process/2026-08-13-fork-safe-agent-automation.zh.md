...
失败、取消、超时、启动失败或 stale 的顶层 Agent Issues、Agent PR Rework、Agent PR CI Repair 与 Agent PR Review run 会进入独立的事件驱动恢复工作流。恢复控制器验证 source run 的 `referenced_workflows` 中精确的可复用 controller SHA，定位匹配的持久 Issue 或 pull request 状态记录，重新检查实时 Issue 状态或精确 PR 版本对，并且最多记录三次重试。传输失败使用有界退避，身份验证或额度失败进入基础设施 dead-letter，畸形协议结果快速失败，任务失败则遵循普通返工预算。评论和标签仍是审计投影；伪造记录不能授权重试。
...
目标仓库使用九个相互独立的 Agent 工作流，分别负责 Issue 分发、精确版本对 PR 审核、可信返工反馈、显式落地、GitHub-hosted 落地协调、仓库监督、有界恢复和健康检查。每个调用方在 `uses` 中固定可复用工作流 revision；控制器 revision、角色 worker 和 runner 选择由控制器持有，不再作为调用方输入。必需 CI 作业在上游仓库中继续使用 larger runner；在 fork 中则选择标准 GitHub-hosted runner，除非仓库变量显式选择 self-hosted 故障转移池。CI 修复与 CI 触发的落地工作流把所有已配置名称声明为字面量 `workflow_run.workflows` 订阅，把收到的名称与 `DSH_AUTOMATION_CI_WORKFLOWS` JSON 数组比对，并要求 `DSH_AUTOMATION_REQUIRED_CHECKS` 中每项检查通过后才落地。包括项目生命周期和 Issue policy 在内的所有特权 PR listener 都使用 `pull_request_target` 并签出默认分支 policy，因此 PR 在进入默认分支前不能替换特权工作流定义。review-submitted 事件不再作为特权输入；自动化 BLOCK 标签只是精确审核 CheckRun 与不可变返工 WorkRequest 的可见投影。

自动化仓库公开一套统一的 Agent Worker 调用与终态回执接口。运行时专用 Adapter 负责启动和观察 DSH Web、ChatGPT Desktop、OpenCode 或 Claude Code；目标工作流把 `review` 与 `change` 角色映射到已配置的 worker id。每个 Worker 都声明其支持的控制器 skill，以及其 Adapter 是否强制执行硬只读审核。配置会拒绝不能同时支持审核、仓库监督和强制只读执行的 review 映射，因此全权限 DSH worker 与通用 `command-json` Adapter 仅用于 change。两个角色分别使用 `agent-reviewer` 与 `agent-change` runner 注册、进程、工作目录、并发组和健康检查作业。

审核 worker 获得精确 base/head checkout，不获得 Actions 凭据，并只做只读检查。控制器只接受 completed Worker 回执中的输出，并把每条阻断 finding 与该精确 diff 中真实存在的文件、行号、逐字摘录和新增行核对。只有实时 base/head 版本对和 checkout 均通过验证后，作业级 Actions token 才会发布 pending 或最终 `codex/review` CheckRun、英文审核评论和投影标签。由于 GitHub 可能规范化其可见 details URL，该 CheckRun 将原始 Actions run URL 存为不透明的外部元数据；落地过程读取该 run 并验证固定的可复用控制器引用。BLOCK 结论记录精确版本对；控制器发布一个不可变、幂等、面向 `change` 角色的返工 WorkRequest 后，审核任务即终止。接收工作流独立启动，校验 WorkRequest 字段、实时 head、审核证据和标签，再调用其配置的变更 worker。已完成失败的配置 CI 工作流会创建一个由 run id 和 attempt 标识的独立请求；只有记录的工作流名称、失败结论、唯一精确 head PR 和当前 head 全部匹配时，控制器才允许变更 worker 检查日志或修改分支。

PASS 结论发出 `dsh-land`，而不是启用长期 auto-merge。落地控制器只接受当前指向仓库默认分支的非草稿 PR，要求精确 base/head PASS 记录，以及由 GitHub Actions App 发布在精确 head 上的每个已配置 CheckRun，在 squash merge 前立即重复这些检查，否则不改变 PR 即退出。由于工作流 token 无法读取仓库管理设置，安装和在线诊断会独立强制执行分支保护要求。成功的已配置 CI run 会重试落地，GitHub-hosted 的 15 分钟协调器则会收口因 mergeability 或检查时序而暂缓的状态，且不会调用模型。

每次向仓库默认分支推送后，协调器都会把同仓库 PR 记录的 base commit 与当前默认分支 commit 比较，并且无论 GitHub 的临时 mergeability 状态如何都会更新陈旧 base。这项变更在 change 角色 runner 上使用经验证的主机 GitHub 凭据，因此 GitHub 会发出普通 pull request 事件并同时重跑 CI 与审核；若使用 Actions 作业 token，这些事件会被抑制。对于不需要更新分支的当前版本，只有缺少可信精确版本对证据时才会显式分发审核。Backlog 与有界恢复使用显式 `repository_dispatch` 事件；backlog 会跳过任何已经带 `agent/dsh` 标签的 Issue，仓库监督在分配该标签后也会立即发送同一个确定性 Issue dispatch。没有声明分支但 marker 验证通过的 CI baseline Issue 会在确定性的 `agent/issue-<number>` 分支上进入同一队列。受保护默认分支上的 listener 与固定的可复用控制器会在调用模型前重新核验实时 Issue 或精确 PR 版本对。`agent-repository-supervision.yml` 每六小时运行固定的仓库监督器，变更预算为五次；它从已验证证据推导 action 指纹，在每次 mutation 前重新读取对应 subject，并用显式 `Depends on #N.` 声明代替仓库专用顺序。`agent-health.yml` 使用 GitHub-hosted 的每小时排队作业 watchdog、本地 replica heartbeat 和每日真实提供方 readiness canary；确定性健康检查与落地协调不会调用模型。
