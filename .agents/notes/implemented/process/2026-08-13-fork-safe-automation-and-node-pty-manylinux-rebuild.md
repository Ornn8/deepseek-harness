...
`build-exe-for-python-sdk.yml` installs the native-build job with pnpm's hoisted linker, resolves the installed node-pty package to its real store directory, and then invokes its npm lifecycle with `npm --prefix` to force a Linux source rebuild. Calling npm at the real directory avoids pnpm revalidating the workspace symlink, while the forced rebuild bypasses the packaged prebuild, invokes node-gyp against the stable job-level node-addon-api layout, and generates the Makefile before it is mounted into the manylinux container. The container then rebuilds the addon against glibc 2.28.
