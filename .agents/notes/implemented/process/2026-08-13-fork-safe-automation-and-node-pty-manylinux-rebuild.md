...
After a push to `master`, reconciliation dispatches review only for open same-repository PRs whose current base/head pair has no completed or pending exact-pair review; it skips draft, behind, and already-covered PRs. The manual health workflow checks each configured worker on its own runner plus the pinned controller and GitHub access without invoking a model.
