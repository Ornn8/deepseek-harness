...
The target repository replaces the combined caller with separate thin workflows for Issue dispatch, exact-pair PR review, trusted rework feedback, explicit landing, and an on-demand health check. Each reusable workflow and its controller checkout use the same full commit SHA from the dedicated `Ornn8/dsh-agent-automation` repository, so upgrading controller behavior is one reviewable target-repository change.

Codex receives an exact base/head checkout with no GitHub credential and performs read-only inspection. A job-scoped Actions token publishes the pending or final `codex/review` status, the English review comment, and projection labels. DSH keeps the host GitHub identity that owns branch writes. A BLOCK verdict records the exact pair, emits an idempotent `dsh-repair` repository dispatch, and also adds `automation/review-blocked`; the pull-request label event is a bootstrap transport while the dispatch receiver has not yet reached the default branch, and the repair controller still validates the live head and review marker before starting a visible DSH session. A completed failed `CI` workflow run creates a separate request keyed by run id and attempt; the controller requires the `CI` workflow name, failure conclusion, matching PR number, and exact current head before DSH may inspect its logs or modify the branch.

A PASS verdict emits `dsh-land` instead of enabling long-lived auto-merge. The landing controller accepts only the current non-draft PR to `master`, requires an exact base/head PASS record and every live branch-protection context at success, repeats those checks immediately before a squash merge, and otherwise exits without changing the PR. A successful `CI` workflow run retries landing after pending checks complete.

After a push to `master`, reconciliation dispatches review only for open same-repository PRs whose current base/head pair has no completed or pending exact-pair review; it skips draft, behind, and already-covered PRs. The manual health workflow checks the pinned controller, DSH Web host, Codex binary, and GitHub access without invoking either model.
...
The Actions publisher separates Codex publication from the host identity used by DSH and landing, but the self-hosted runner and the latter two operations still share one persistent host GitHub login. Full process isolation and distinct GitHub Apps remain security hardening work; this change does not claim that separation.
