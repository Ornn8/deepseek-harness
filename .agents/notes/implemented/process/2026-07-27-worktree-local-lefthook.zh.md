...
升级格式 0 之前，安装程序会拒绝共用配置中直接设置的 `extensions.*`；它还会拒绝直接设置的 `core.worktree` 或 `core.bare=true`，以及启用扩展后将被激活的非空休眠 worktree 配置。迁移会移除直接设置的 `core.bare=false`，因为 false 是 Git 的默认值。共用仓库配置和每个已有的 `config.worktree` 都必须是常规文件。这些检查会禁用 include 展开，因为 Git 的仓库格式解析器也会忽略 include 目标。仓库级锁会串行化迁移和钩子写入；释放时，锁的进程 ID、随机令牌、文件身份和完整内容必须仍然匹配。所属进程已结束或内容无效的锁必须人工介入恢复，而不能自动强制解除。在 Windows 上，与并发释放竞争的独占创建会在已释放的锁名处于删除挂起状态时报告 EPERM；在名称仍处于删除挂起状态时，分类所用的 lstat 与锁文件读取也可能报告同样的 EPERM，而释放一旦完成名称便会直接消失。安装程序将锁路径操作中的任何 Windows EPERM 都视为瞬时竞争，并在 30 秒截止时间内重试创建。
