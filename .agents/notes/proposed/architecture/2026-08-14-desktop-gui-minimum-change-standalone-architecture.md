...
The rendered baseline records committed screenshots of those same states at a fixed viewport on the Chromium major the WebUI is tested against, with the recording conditions (viewport size, platform, theme) stated beside each capture; later work must match these captures, not redesign them. The real-host smoke already writes full-page screenshots as comparison evidence ([smoke-real.e2e.ts](../../../../apps/web/tests/smoke-real.e2e.ts)); the baseline records the same kind of capture keylessly.

Static art gets an inventory check of its own: every upstream visual/static resource must stay present and resolvable in the packaged desktop app — [`apps/web/public/**`](../../../../apps/web/public/) (`favicon.svg`, `manifest.webmanifest`) and the imported SVG/image/font assets Vite emits into the dist (`assets/fonts/*` for KaTeX faces, plus icons and images). The baseline records that inventory; a later desktop deliverable proves each entry resolves in the packaged app.
...
- A standalone window renders the official WebUI with no visual or behavioral difference from the browser surface, evidenced by the same keyless replay goldens, the committed screenshot baseline, and the static-asset resolution check.
