...
该包不了解任何 harness 概念，也不提供任何文件服务：`/api` HTTP 桥接与下行 WebSocket 是 connection 插件的 route，插件 bundle 与 HMR（热模块替换）事件流是 modules／hmr 插件的 route，dist 服务则属于 fallback 持有者。upgrade handler 拥有协议握手与连接内容；webserver 只交付原始 socket 与 request。`host` 只接受 `127.0.0.1`（默认安全姿态）和 `0.0.0.0`（有意向网络开放）。独立 GUI 外壳（`apps/desktop`）加载本服务器服务的同一回环源；通过 `file://` 的 IPC 载体仍是待定的传输决策。该包从不打印内容；URL 行属于 shell。

监听失败（EADDRINUSE……）会从激活过程抛出，并以绑定诊断信息拒绝 Loader 组合；失败的候选 fiber 会被 dispose（资源释放）。处理 HTTP 请求时发生的拒绝会被记录为 warning 并隔离处理，绝不会退出进程。被归类为客户输入错误的拒绝（`BadRequestError`，由 route 或 fallback handler 在遇到格式错误的百分号转义或无法解析的请求目标时抛出）会应答 400；任何其他拒绝（内部 route／fallback 失败、静态读取失败、客户端在请求体传输中途断开）都是内部故障，应答 500；若响应头已经发出，则改为销毁 socket。upgrade handler 抛错或升级 socket 出现传输错误时，会记录 warning 并销毁对应 socket。资源释放会启动 `close()` 与 `closeAllConnections()`，销毁所有受跟踪的升级 socket，并仅在 HTTP server 与这些 socket 均已关闭后返回。
